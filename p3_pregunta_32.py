...
# tam_diccionario=[5, 15, 25, 50, 75, 100, 125, 150, 175, 200]#ejercicio 3.2.1
tam_diccionario=[50]#ejercicio 3.2.1 OPTIMO
...
for tam_diccionario_aux in tam_diccionario:
    ...
    # Sexto: Usar SVM para entrenar
    #tipo='linear'
    #tipo='rbf'
    tipo='poly'

    vectores_hog = svm.SVC(kernel = tipo)
    vectores_hog.fit(descriptores_BOW_train_hog, y_train) # Entrenamiento de SVM
    Rendimiento_test_hog = vectores_hog.score(descriptores_BOW_test_hog, y_test)
    Rendimiento_train_hog = vectores_hog.score(descriptores_BOW_train_hog, y_train)

    vectores_tiny = svm.SVC(kernel = 'linear')
    vectores_tiny.fit(descriptores_BOW_train_tiny, y_train) # Entrenamiento de SVM
    Rendimiento_test_tiny = vectores_tiny.score(descriptores_BOW_test_tiny, y_test)
    Rendimiento_train_tiny = vectores_tiny.score(descriptores_BOW_train_tiny, y_train)
...
y_pred = vectores_hog.predict(descriptores_BOW_test_hog)
y_pred_words = []
...
confusion_matrix = create_results_webpage(x_train, x_test, y_train_words, y_test_words, categories, abbr_categories, y_pred_words, 'pregunta_32')
