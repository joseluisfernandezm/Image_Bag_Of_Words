...
#tam_diccionario=[50]#ejercicio 3.1.1
#tam_diccionario=[5, 15, 25, 50, 75, 100, 125, 150, 175, 200]#ejercicio 3.1.2
tam_diccionario=[200]#ejercicio 3.1.3 OPTIMO
...
x=datos.filenames
y=datos.target
[x_train, x_test, y_train, y_test] = train_test_split(x, y, test_size=train_test, random_state = 42) 

# Tercero: HOG
#tam=100 #para ejercicio base
tam=64 # para ejercicio 3.1.1
...
for tam_diccionario_aux in tam_diccionario:

    # Cuarto: Vocabulario BOW con datos train
    max_iter=10
    vocabulario_train_hog =  construir_vocabulario(list_img_desc_hog_train, vocab_size = tam_diccionario_aux, max_iter = max_iter)

    vocabulario_train_tiny =  construir_vocabulario(list_img_desc_tiny_train, vocab_size = tam_diccionario_aux, max_iter = max_iter)
    ...
    # Sexto: Usar KNN para entrenar
    #k=5 # numero de vecinos original
    k=21# numero de vecinos ejercicio 3.1.3

    vecinos_hog = KNeighborsClassifier(n_neighbors = k)
    vecinos_hog.fit(descriptores_BOW_train_hog, y_train) # Entrenamiento de KNN, aunque en realidad no entrena como tal

    
    Rendimiento_test_hog = vecinos_hog.score(descriptores_BOW_test_hog, y_test)
    Rendimiento_train_hog = vecinos_hog.score(descriptores_BOW_train_hog, y_train)


    vecinos_tiny = KNeighborsClassifier(n_neighbors = k)
    vecinos_tiny.fit(descriptores_BOW_train_tiny, y_train) # Entrenamiento de KNN, aunque en realidad no entrena como tal

    Rendimiento_test_tiny = vecinos_tiny.score(descriptores_BOW_test_tiny, y_test)
    Rendimiento_train_tiny = vecinos_tiny.score(descriptores_BOW_train_tiny, y_train)

    print('Tamaño Diccionario = ',tam_diccionario_aux)
...
y_pred = vecinos_hog.predict(descriptores_BOW_test_hog)
y_pred_words = []
...
confusion_matrix = create_results_webpage(x_train, x_test, y_train_words, y_test_words, categories, abbr_categories, y_pred_words, 'pregunta_31')
